      ...
      if (findItem(state.data, action.payload)) {
        state.data = state.data.map((item) =>
          checkItem(item, action.payload)
            ? { ...item, quantity: item.quantity + item.pack }
            : item,
        );
      } else {
    ...
    removeItem: (state, action) => {
      ...
      if (findItemInner && findItemInner.quantity > findItemInner.pack) {
        state.data = [
          ...
          {
            ...action.payload,
            quantity: action.payload.quantity - action.payload.pack,
          },
        ];
      } else {
...
export const { addItem, removeItem, clearItem, clearShopCart } =
