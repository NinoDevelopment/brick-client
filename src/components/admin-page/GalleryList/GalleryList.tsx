...
const GalleryList = () => {
   ...
   return (
      ...
      <header className={styles.header}>
            <h1>Галлерея ({data.loading ? "Обновление..." : data.categories.length})</h1>
         </header>

         {!data?.categories?.length && <h5 className={styles.noItems}>Список пуст!</h5>}

         <div className={styles.content}>
            {
               !!data?.categories?.length &&
               data.categories.map(elem => <GalleryCard key={elem.categoryId} data={elem} />)
            }
         </div>
      </div>
   );
